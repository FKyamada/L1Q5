...
void Lista::remove_rec(int elem)
{
	No*pAux = primeiro, *apagar;
	
	if (eVazia())
	{
		printf("A LISTA ESTÁ VAZIA\n");
	}
	else if(existe(elem)== false)
	{
		printf("O ELEMENTO ESCOLHIDO NAO EXISTE, digite um numero valido\n");
	}
	else
	{
		while (pAux != NULL)
		{
		if (pAux->getInfo() == elem)
		{
		apagar = pAux;
		if (pAux->getProximo() != NULL)
		{
			primeiro = pAux->getProximo();
			pAux = primeiro;
		}
		else
		{
		...
		else
		{
			if (pAux->getProximo() != NULL)
			{
				if (pAux->getProximo()->getInfo() == elem)
				{
					apagar = pAux->getProximo();
					pAux->setProximo(apagar->getProximo());
					apagar->~No();
					delete(apagar);
					pAux = pAux->getProximo();
				}
